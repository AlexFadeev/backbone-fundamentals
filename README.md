...
Корректировки этого материала всегда приветсвуются, и я надеюсь, что вместе мы обеспечим сообщество актуальным на сегодняшний день вспомогательным материалом.
...
Шаблоны полезны, поскольку являются набором практик, которые основываются на коллективном опыте квалифицированных разработчиков, решавших подобные проблемы не один раз. И хотя разработчики 10 или 20 лет назад не использовали теже програмные языки при реализации шаблонов проектирования в своих проектах, из их усилий мы можем почерпнуть для себя очень много уроков.
...
Чтобы понять модели лучше, давайте представим, что у нас есть JavaScript-приложение: фотогалерея. В фотогалерее фотографии необходима собственная модель, поскольку она представляет собой уникальный вид предметно-ориентированных данных. Модель фотографии может содержать атрибуты, такие как заголовок, источник изображения и другие мета-данные. Конкретные фотографии будут храниться в экземплярах модели. Вот пример простой модели фотографии с использованием Backbone.js:
...
Вот так бы мы могли группировать модели фотографий в упрощенной Коллекции Backbone:
...
Пользовательское взаимодействие с представлениями обычно означает чтение и редактирование данных модели. Например, в нашем примере приложения фотогалереи, представление модели может происходить посредством большого изображения, заголовка и списка тегов. Редактирование модели может быть произведено через "редактирование" представления, в котором пользователь изменил заголовок, теги или другие мета-данные для выбранной фотографии.
...
Затем добавляем ```photoModel``` в обратный вызов (колбэк) нашей функции ```render()``` в качестве подписчика, так что с помощью модели "Наблюдатель" представление теперь может реагировать на изменения модели и обновляться.

Вы можете задаться вопросом, а где же здесь вступает в игру взаимодействие с пользователем. Однако не в ответственности представления решать, что делать дальше при клике пользователя на любом его элементе. Это решение принимает Контроллер. В нашем примере реализации это достигается с помощью добавления слушателя события ```click``` для ```photoEl```, который будет делегировать управление поведением контроллеру, передавая информацию о модели.

Преимуществом такой архитектуры является разделение роли каждого из компонентов для обеспечения необходимого функционирования приложения.
...
В Микрошаблонах вы можете также использовать и двойные фигурные скобки (то есть ```{{}}```) (или любой другой способ обозначения переменных в шаблонах, который представляется вам наиболее комфортным). В случае с фигурными скобками, нужно всего навсего указать в настройках Underscore атрибут ```templateSettings``` следующим образом:
